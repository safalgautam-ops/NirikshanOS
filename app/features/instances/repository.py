...
async def list_instances() -> list[dict]:
    return await db.table("instances").order_by("display_name", "asc").all(allow_full_table=True)


async def list_active_instances() -> list[dict]:
    return await (
        db.table("instances").where("is_active", 1).order_by("display_name", "asc").all(allow_full_table=True)
    )


async def list_ready_instances() -> list[dict]:
    """Active instances whose image has actually been confirmed built (`docker image inspect` succeeded — see workers/worker_main.py's _check_instance_image)."""
    return await (
        db.table("instances")
        .where("is_active", 1)
...
async def get_usage_counts(instance_id: str) -> dict:
    """How many rows in each dependent table currently reference this instance - shown in the delete confirmation so an admin knows the blast radius before confirming, since two of these three references (modules, plan grants) change silently on delete (SET NULL / CASCADE) rather than blocking it."""
    modules = await db.table("analysis_module_defs").where("instance_id", instance_id).count()
    plans = await db.table("plan_instances").where("instance_id", instance_id).count()
    test_runs = await db.table("module_test_runs").where("instance_id", instance_id).count()
    return {"modules": modules, "plans": plans, "test_runs": test_runs}


async def clear_test_runs_for_instance(instance_id: str) -> int:
    """Deletes every module_test_runs row referencing this instance - the one reference that actually blocks instance deletion (module_test_runs has no ON DELETE rule, unlike plan_instances/analysis_module_defs which cascade/null out silently - see get_usage_counts)."""
    runs = await (

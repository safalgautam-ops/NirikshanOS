...
(anyone holding a role with at least one granted permission) are exempt;
onboarding doesn't apply to them.
...
STATE_ACTIVE = "active"  # holds permissions, or member of an approved org


async def get_organization_state(user_id: str) -> str:
    if await get_user_permission_names(user_id):

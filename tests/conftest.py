...
def app():
    """One Flask app for the whole test session - mirrors production: one process, one persistent AsyncRuntime loop, one DB pool."""
...
@pytest.fixture(scope="session")
def run_async(app):
    """Run an awaitable on the app's own persistent event loop - lets integration tests call async service/repository functions directly, without a second, conflicting event loop or DB pool (see AsyncFlask.run_async in app/core/async_runtime.py)."""
...
def _flush_test_redis(app, run_async):
    """Every test starts against an empty Redis keyspace (db 1, isolated from dev's db 0 - see the REDIS_URL override above)."""
...
def unique(prefix: str = "t") -> str:
    """A short, collision-free identifier for anything with a uniqueness constraint (email, slug, ...) so tests never collide with each other or with a previous run against the same disposable database."""
    return f"{prefix}_{uuid.uuid4().hex[:12]}"


@pytest.fixture()
def make_user(run_async):
    """Creates a real, fully-active user row (bypassing the email-OTP activation flow, which is out of this suite's scope) and returns its id + the plaintext password, so a test can log in with it."""
...
def make_org(run_async):
    """Creates a real, approved organization directly via the repository - bypassing the onboarding wizard's document-upload steps, which are out of this suite's scope."""
...
def grant_permission(run_async):
    """Creates a fresh role holding exactly one platform permission (resource.action, e.g. "plans.view") and assigns it to a user - the real path a permission actually reaches a user through (report §5.4)."""

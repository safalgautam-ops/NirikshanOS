"""E2E test: a real, ordinary (non-staff) logged-in user cannot reach an admin-only page through the actual browser - the negative-path counterpart to test_user_journey.py, confirming report §5.4's RBAC claim holds at the rendered-page level, not only at the route level (already covered in tests/integration/test_rbac_permissions.py)."""

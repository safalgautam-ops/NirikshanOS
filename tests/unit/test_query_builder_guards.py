"""Unit tests: the custom query builder's write-safety guards (app/core/db/orm.py - referenced in report §4.3/Appendix E2)."""
...
async def test_delete_with_where_passes_the_guard():
    """The guard only checks that a condition exists - it should not raise once one is present (whether the row exists is a DB-layer concern, covered by the integration tier, not this unit test)."""

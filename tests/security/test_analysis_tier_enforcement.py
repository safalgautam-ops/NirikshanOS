"""Security test: server-side plan-tier enforcement for running analysis modules (app/features/analysis/policy.py::check_can_run)."""
...
def _seed_enterprise_module(run_async) -> str:
    """A real, ready-to-run instance + module def, at tier "enterprise" - the highest tier, so only the Enterprise plan's allowed_tiers includes it (see migrations/019 for the four plans' seeded allowed_tiers)."""
    instance_id = unique("instance")
    run_async(
        db.table("instances").create(
            {
                "id": instance_id,
    ...
    run_async(
        db.table("analysis_module_defs").create(
            {
                "id": module_id,
...
def test_enterprise_plan_org_can_submit_the_same_module(client, make_user, make_org, run_async):
    """Same module, same request shape, only the plan differs - confirms the 403 above is really about the plan tier and not something else about the request (a wrong module id, missing evidence, etc)."""
    module_id = _seed_enterprise_module(run_async)
    owner = make_user()
    org = make_org(created_by=owner["id"])
    run_async(add_org_member(org["id"], owner["id"]))
